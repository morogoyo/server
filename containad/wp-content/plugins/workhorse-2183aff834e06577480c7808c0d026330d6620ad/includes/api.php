...
if (isset($_GET['api']) && $_GET['api'] == 'workhorse') {
    ...
    elseif ($act == 'count_posts') {
        ...
        $tags = workhorse_search_geotags(array(
                $_POST['title'], $_POST['content'], $_POST['custom_title'], $_POST['custom_description'], $_POST['custom_keywords'], $_POST['permalink'], $_POST['tags']
            ));
        ...
        if ($geo_iterations == 0) $iterations = max(1, Spintax::count(Spintax::parse($title)));
        else $iterations = $geo_iterations;
    ...
    elseif ($act == 'shortcode') {
    	...
    	$sql = $wpdb->prepare("INSERT INTO {$wpdb->prefix}workhorse_shortcodes (shortcode, type, content) VALUES (%s, %s, %s)", array($shortcode, 'static', $media));
